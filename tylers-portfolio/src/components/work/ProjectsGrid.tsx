 ...
 */
export default function ProjectsGrid({ projects, href, className }: ProjectsGridProps) {
  const buildHref = href ?? ((p: Project) => `/portfolio/${encodeURIComponent(p.id)}`)
  return (
    <ul
      className={cn(
        'grid list-none grid-cols-5 gap-0 p-0',
        className,
      )}
    >
