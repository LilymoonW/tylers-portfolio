...
export default function ProjectPageTemplate({ project }: { project: Project }) {
  ...
  const roadsLeadBefore = roadsLeadNeedsBreak ? titleUpper.slice(0, roadsLeadIdx).trimEnd() : ''
  ...
  const titleLineGroups: string[][] = titleNeedsBreak
    ? [chunkWordsByTwo(`${titleBeforeBreak} 32`), chunkWordsByTwo(`TEAM ${titleAfterBreak}`)]
    : roadsLeadNeedsBreak
  ...
  const socialEmbed =
    !project.videoSrc && project.embedUrl ? getSocialEmbedSrc(project.embedUrl) : null
        ...
        >
          <span className="contents portrait:hidden">
            {titleLineGroups.flatMap((group, gi) =>
              group.map((line, li) => (
                <span
                  key={`landscape-${gi}-${li}`}
                  className={cn(justifyLineClass, gi > 0 && li === 0 && 'mt-[0.35em]')}
                >
                  {line}
                </span>
              )),
          ...
          <span className="hidden portrait:contents">
            ...
            {portraitLine2.trim().length > 0 ? (
              <span className={cn(justifyLineClass, 'mt-[0.28em]')}>{portraitLine2}</span>
            ) : null}
          </span>
        </h1>
      </header>
        ...
        <TemplateSection label="Media">
          <div className="flex w-full justify-center">
            {project.videoSrc ? (
                ...
                <>
                  ...
                  >
                    <Link
                      ...
                      data-cursor={project.mediaExternalAffordance ? 'expand' : 'play'}
                      aria-label={
              ...
              >
                <p className="text-center text-xs text-black/45 md:text-sm">VIDEO GOES HERE</p>
              </div>
            )}
          </div>
        </TemplateSection>
          ...
          <TemplateSection label="Videographers">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {project.videographers.map((name, i) => (
                ...
                >
                  <p className="mt-1 text-sm uppercase text-black/55 md:text-base">{name}</p>
                </div>
              ))}
            </div>
          </TemplateSection>
